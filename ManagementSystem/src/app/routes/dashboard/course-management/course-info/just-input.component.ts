...
@Component({
    ...
    <form (ngSubmit)="confirm()" #form="ngForm" aria-labelledby="title">
        ...
        <div class="inputs">
            <input *ngIf="type!='textarea'" type="{{type}}" required [pattern]='pattern' 
                   [(ngModel)]='value' nbInput fullWidth placeholder="{{name}}" name='valuex'>
            <textarea *ngIf="type=='textarea'" rows='5' nbInput fullWidth placeholder="{{name}}" 
                   [(ngModel)]='value' [pattern]='pattern' required name="valuex" ></textarea>
        </div>
        <div class="buttons">
...
export class JustInputComponent implements OnInit {
    @Input()
    message: string;
    @Input()
    pattern: string;
